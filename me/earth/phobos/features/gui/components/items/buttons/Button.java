...
extends Item {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        RenderUtil.drawRect(this.x, this.y, this.x + (float)this.width, this.y + (float)this.height - 0.5f, this.getState() ? (!this.isHovering(mouseX, mouseY) ? Oyvey.colorManager.getColorWithAlpha(Oyvey.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : Oyvey.colorManager.getColorWithAlpha(Oyvey.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue())) : (!this.isHovering(mouseX, mouseY) ? 0x20FFFFFF : -2007673515));
