...
extends Module {
    private final Setting<?> mode = this.register(new Setting<Mode>("Mode", Mode.Fast));
    ...
    public Setting rotation = this.register(new Setting<Object>("Rotate", Boolean.valueOf(false), v -> this.mode.getValue() == Mode.Fast));
    private int lastY;
    ...
    public Scaffold() {
        ...
        this.swing = this.register(new Setting<Object>("Swing Arm", Boolean.valueOf(false), v -> this.mode.getValue() == Mode.Legit));
        this.bSwitch = this.register(new Setting<Object>("Switch", Boolean.valueOf(false), v -> this.mode.getValue() == Mode.Legit));
        this.center = this.register(new Setting<Object>("Center", Boolean.valueOf(false), v -> this.mode.getValue() == Mode.Legit));
        this.keepY = this.register(new Setting<Object>("KeepYLevel", Boolean.valueOf(false), v -> this.mode.getValue() == Mode.Legit));
        this.sprint = this.register(new Setting<Object>("UseSprint", Boolean.valueOf(true), v -> this.mode.getValue() == Mode.Legit));
        this.replenishBlocks = this.register(new Setting<Object>("ReplenishBlocks", Boolean.valueOf(true), v -> this.mode.getValue() == Mode.Legit));
        this.down = this.register(new Setting<Object>("Down", Boolean.valueOf(false), v -> this.mode.getValue() == Mode.Legit));
        this.expand = this.register(new Setting<Object>("Expand", Float.valueOf(1.0f), Float.valueOf(0.0f), Float.valueOf(6.0f), v -> this.mode.getValue() == Mode.Legit));
    ...
    public void onUpdate() {
        if (this.mode.getValue() == Mode.Legit) {
            if (Oyvey.moduleManager.isModuleEnabled("Sprint") && (((Boolean)this.down.getValue()).booleanValue() && Scaffold.mc.field_71474_y.field_74311_E.func_151470_d() || !((Boolean)this.sprint.getValue()).booleanValue())) {
