...
extends Module {
    ...
    public void onEnable() {
        ...
        if (this.shouldServer()) {
            AutoTrap.mc.field_71439_g.field_71174_a.func_147297_a((Packet)new CPacketChatMessage("@Serverprefix" + ClickGui.getInstance().prefix.getValue()));
            AutoTrap.mc.field_71439_g.field_71174_a.func_147297_a((Packet)new CPacketChatMessage("@Server" + ClickGui.getInstance().prefix.getValue() + "module AutoTrap set Enabled true"));
        }
    }
    ...
    public void onDisable() {
        ...
        if (this.shouldServer()) {
            AutoTrap.mc.field_71439_g.field_71174_a.func_147297_a((Packet)new CPacketChatMessage("@Serverprefix" + ClickGui.getInstance().prefix.getValue()));
            AutoTrap.mc.field_71439_g.field_71174_a.func_147297_a((Packet)new CPacketChatMessage("@Server" + ClickGui.getInstance().prefix.getValue() + "module AutoTrap set Enabled false"));
            return;
        }
        isPlacing = false;
    ...
    private void doStaticTrap() {
        List<Vec3d> placeTargets = EntityUtil.targets(this.target.func_174791_d(), this.antiScaffold.getValue(), this.antiStep.getValue(), this.legs.getValue(), this.platform.getValue(), this.antiDrop.getValue(), this.raytrace.getValue(), this.face.getValue());
        this.placeList(placeTargets);
        this.currentPlaceList = placeTargets;
    }
    ...
    private void placeList(List<Vec3d> list) {
        ...
        for (Vec3d vec3d3 : list) {
            ...
            if (this.entityCheck.getValue().booleanValue() && placeability == 1 && (this.switchMode.getValue() == InventoryUtil.Switch.SILENT || this.retries.get((Object)position) < this.retryer.getValue())) {
    ...
    private boolean check() {
        ...
        if (this.disable.getValue().booleanValue() && this.startPos != null && !this.startPos.equals((Object)EntityUtil.getRoundedBlockPos((Entity)AutoTrap.mc.field_71439_g))) {
            this.disable();
            return true;
        }
        ...
        return this.target == null || Oyvey.moduleManager.isModuleEnabled("Freecam") && this.freecam.getValue() == false || !this.timer.passedMs(this.delay.getValue().intValue()) || this.switchMode.getValue() == InventoryUtil.Switch.NONE && AutoTrap.mc.field_71439_g.field_71071_by.field_70461_c != InventoryUtil.findHotbarBlock(BlockObsidian.class);
    }
